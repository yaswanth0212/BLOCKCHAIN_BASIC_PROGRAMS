...
contract retailar is ERC721 {
    ...
    struct retailar_details{ //structure declaration for producers
 ...
 mapping (uint256 => retailar_details) files; //mapping producers_details
...
//function Calling for geting product details
 function getProductDetails(uint256 productId) public view returns (string memory,string memory,string memory,string memory,string memory,string memory) {
     if(_exists(productId)&&(ownerOf(productId)== msg.sender||msg.sender == owner)){
     
    retailar_details storage sm = files[productId];
        return (sm.food_type,sm.food_name,sm.food_place,sm.food_pesticides, sm.producers_name,sm.retailar_name);
     }
     else
     {
            ...
            return(foodtype,foodname,foodplace,foodpesticides,producersname,retailarname);
     }
